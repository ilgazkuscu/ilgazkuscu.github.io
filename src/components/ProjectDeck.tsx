  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <div className="fixed bottom-5 left-1/2 z-30 flex -translate-x-1/2 items-center gap-2 rounded-full border border-cream/10 bg-ink/60 px-3 py-2 backdrop-blur md:bottom-8">
...
function DeckSlide({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <div className="relative min-h-[360px] overflow-hidden rounded-[2rem] border border-cream/10 bg-gradient-to-br from-cream/[0.08] via-copper/10 to-transparent p-6 shadow-2xl shadow-black/30 md:min-h-[500px]">
      ...
      <div className="relative flex h-full flex-col justify-between gap-8">
        <div className="flex items-center justify-between text-xs uppercase tracking-[0.24em] text-cream/45">
          <span>{project.id}</span>
          <span>{project.status || project.where || 'Portfolio deck'}</span>
        </div>
